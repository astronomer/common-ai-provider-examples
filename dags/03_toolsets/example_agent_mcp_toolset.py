...
`@task.agent` with `MCPToolset`. The `mcp_stdio_space` connection launches
`include/mcp_server/space_server.py` as a subprocess, and the agent calls
`list_planets`, `list_spacecraft` and `count_stations_by_planet` over MCP.
"""
