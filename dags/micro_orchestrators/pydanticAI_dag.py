"""
## support_ticket_pydantic_ai

Same support-ticket flow as `ai_support_ticket_system`, but the draft step
uses the pydantic-ai library directly inside a plain `@task` instead of the
common-ai provider's `@task.agent`. Shows what the provider abstracts away:
agent construction, model selection, and output handling live in user code.
"""
...
@dag(
    ...
    schedule=None,
    catchup=False,
    tags=["micro-orchestrator", "pydantic-ai", "example", "space", "support-ticket"],
    doc_md=__doc__,
)
def support_ticket_pydantic_ai():
    @task
    ...
    def generate_ai_response(ticket: dict) -> dict:
        ...
        result = agent.run_sync(build_user_prompt(ticket))
