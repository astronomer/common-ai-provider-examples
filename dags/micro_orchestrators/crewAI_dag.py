"""
## support_ticket_crewai

Same support-ticket flow as `ai_support_ticket_system`, but the draft step
assembles a single-agent CrewAI crew inside a plain `@task`. CrewAI's
role/goal/backstory slots map onto the shared prompt: the system prompt
becomes the agent goal, the user prompt becomes the crew task description.
CrewAI telemetry is disabled in-task via CREWAI_DISABLE_TELEMETRY.
"""
...
@dag(
    ...
    schedule=None,
    catchup=False,
    tags=["micro-orchestrator", "crewai", "example", "space", "support-ticket"],
    doc_md=__doc__,
)
def support_ticket_crewai():
    @task
    ...
    def generate_ai_response(ticket: dict) -> dict:
        ...
        @crew_tool("lookup_shipment")
        def lookup_shipment_tool(shipment_id: str) -> dict:
        ...
        support_agent = Agent(
            ...
            goal=SYSTEM_PROMPT,
            backstory=(
                "Experienced customer support agent at Cosmic Freight, an "
                "interplanetary shipping company."
            ),
            llm="openai/gpt-5-mini",
            tools=[lookup_shipment_tool],
        )
        draft = CrewTask(
            description=build_user_prompt(ticket),
            expected_output=(
                "A TicketResponse with summary, response, priority, "
                "confidence_score and suggested_tags."
            ),
            output_pydantic=TicketResponse,
            agent=support_agent,
        )
        crew = Crew(agents=[support_agent], tasks=[draft])
        result = crew.kickoff()
