"""
## support_ticket_temporal

Same support-ticket flow as `ai_support_ticket_system`, but the draft step
runs a Temporal workflow. The Airflow task connects to the Temporal dev
server (`temporal:7233`, see docker-compose.override.yml), starts an
in-process worker, and executes `TicketResponseWorkflow`, which drives the
LLM tool-calling loop as Temporal activities. Durable execution lives in
Temporal here, where the common-ai provider would use `durable=True`.
"""
...
TEMPORAL_TASK_QUEUE = "support-ticket-queue"
...
@activity.defn
async def lookup_shipment_activity(shipment_id: str) -> dict:
    return lookup_shipment(shipment_id)


@workflow.defn
class TicketResponseWorkflow:
    @workflow.run
    async def run(self, ticket: dict) -> dict:
        ...
        while True:
            ...
            if not message["tool_calls"]:
                return message["parsed"] or json.loads(message["content"])
            messages.append(
                {
                    "role": "assistant",
                    "content": message["content"],
...
@dag(
    ...
    schedule=None,
    catchup=False,
    tags=["micro-orchestrator", "temporal", "example", "space", "support-ticket"],
    doc_md=__doc__,
)
def support_ticket_temporal():
    @task
    ...
    def generate_ai_response(ticket: dict) -> dict:
        async def _run() -> dict:
            client = await asyncio.wait_for(
                Client.connect(TEMPORAL_ADDRESS), timeout=10
            )
            async with Worker(
                client,
                task_queue=TEMPORAL_TASK_QUEUE,
                workflows=[TicketResponseWorkflow],
                activities=[call_llm, lookup_shipment_activity],
                workflow_runner=UnsandboxedWorkflowRunner(),
            ):
                return await client.execute_workflow(
                    TicketResponseWorkflow.run,
                    ticket,
                    id=f"support-ticket-{ticket['ticket_id']}-{uuid.uuid4().hex[:8]}",
                    task_queue=TEMPORAL_TASK_QUEUE,
                )

        result = asyncio.run(_run())
        return TicketResponse.model_validate(result).model_dump()
