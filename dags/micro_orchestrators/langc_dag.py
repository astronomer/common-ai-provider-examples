"""
## support_ticket_langgraph

Same support-ticket flow as `ai_support_ticket_system`, but the draft step
builds a LangGraph ReAct agent (`create_react_agent` + `langchain-openai`)
inside a plain `@task`. LangGraph was chosen over plain LangChain chains and
LangSmith because it is the agent-orchestration library of that family.
"""
...
@dag(
    ...
    schedule=None,
    catchup=False,
    tags=["micro-orchestrator", "langgraph", "example", "space", "support-ticket"],
    doc_md=__doc__,
)
def support_ticket_langgraph():
    @task
