...
`@task.agent` pointed at a custom pydantic-ai `AbstractToolset`.
`IssOpenNotifyToolset` calls the Open Notify API for the ISS position, and the
agent turns the reading into a structured `IssTelemetryReport`.

This one also sets most of the agent constructor: `model_id`,
`enable_tool_logging`, `agent_params`, `durable` and the HITL settings. The user
prompt is whatever string the decorated function returns, the same contract as
`AgentOperator.prompt`.
"""
