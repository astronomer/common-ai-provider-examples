"""
## support_ticket_common_ai

The baseline for the six-way framework comparison. Fetch an open support
ticket, draft a structured `TicketResponse` using a `lookup_shipment` tool,
then put it in front of a person with `HITLBranchOperator`, who can approve it,
reply by hand, or escalate.

The draft step is `@task.agent` from the common-ai provider. The other five
Dags in this folder run the identical flow with a different library in that one
task, so diffing any of them against this file shows what the provider is doing
for you.
"""
...
shipment_toolset = FunctionToolset(id="shipment_lookup", tools=[lookup_shipment])
...
def support_ticket_common_ai():
    ...
    @task.agent(
        ...
        toolsets=[shipment_toolset],
    )
    def generate_ai_response(ticket: dict) -> str:
