...
The `support_ticket_common_ai` flow with the draft step running as a Temporal
workflow. The Airflow task connects to the Temporal dev server
(`temporal:7233`, see docker-compose.override.yml), starts an in-process
worker, and executes `TicketResponseWorkflow`, which drives the tool-calling
loop as Temporal activities.

Temporal is doing the durable execution here, in place of the provider's
`durable=True`.
"""
